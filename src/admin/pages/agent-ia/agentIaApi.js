// ══════════════════════════════════════════════════════════
//  src/admin/pages/agent-ia/agentIaApi.js
//  Client API pour l'Agent IA
// ══════════════════════════════════════════════════════════
...
export async function listConversations({ limit = 50, offset = 0 } = {}) {
  ...
  const res = await fetch(`${BASE}/admin/agent-ia/conversations?${q}`, {
    headers: authHeaders(),
  });
  return handle(res);
}
...
export async function getConversation(id) {
  const res = await fetch(`${BASE}/admin/agent-ia/conversations/${id}`, {
    headers: authHeaders(),
  });
  return handle(res);
}
...
export async function clearAllConversations() {
  const res = await fetch(`${BASE}/admin/agent-ia/conversations`, {
    method: "DELETE",
    headers: authHeaders(),
  });
  return handle(res);
...
export async function sendMessage({ message, conversation_id = null }) {
  const res = await fetch(`${BASE}/admin/agent-ia/chat`, {
    method: "POST",
    headers: authHeaders(),
...
export async function sendMessageStream({ message, conversation_id = null }, callbacks = {}) {
  ...
  try {
    const res = await fetch(`${BASE}/admin/agent-ia/chat/stream`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ message, conversation_id }),
    ...
    while (true) {
      ...
      // Découpage par event SSE (séparés par \n\n)
