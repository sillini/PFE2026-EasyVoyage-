...
// ✅ fetchJson lève une vraie erreur si la réponse n'est pas OK
    ...
    <div style={{ display: "flex", gap: 2 }}>
      {[1, 2, 3, 4, 5].map(i => (
        <svg key={i} width="14" height="14" viewBox="0 0 24 24"
          fill={i <= n ? "#C4973A" : "none"}
          stroke={i <= n ? "#C4973A" : "#DDE3EC"}
          strokeWidth="1.5">
...
export default function AdminHotelDetail({ hotelId, onBack }) {
  ...
  // ✅ Gestion d'erreur correcte : l'hôtel plante → affiche l'erreur
  const loadAll = async () => {
    setLoading(true);
    setError("");
    try {
  ...
  return (
    <div className="ahd-root">
      ...
      <div className="ahd-hero">
        <div className="ahd-hero-img"
          ...
          {mainImg
            ...
            : <div className="ahd-hero-no-img">🏨</div>
          }
        ...
        <div className="ahd-hero-stats">
          ...
          <div className="ahd-stat">
            <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.5rem", fontWeight: 700, color: "#1A3F63" }}>{chambres.length}</span>
            <span style={{ fontFamily: "'Lato',sans-serif", fontSize: "0.72rem", color: "#8A9BB0", textTransform: "uppercase", letterSpacing: "0.5px" }}>Chambre{chambres.length > 1 ? "s" : ""}</span>
            <div className="ahd-stat-sep"/>
          </div>
          <div className="ahd-stat">
            <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.5rem", fontWeight: 700, color: "#1A3F63" }}>{images.length}</span>
            <span style={{ fontFamily: "'Lato',sans-serif", fontSize: "0.72rem", color: "#8A9BB0", textTransform: "uppercase", letterSpacing: "0.5px" }}>Photo{images.length > 1 ? "s" : ""}</span>
          </div>
        </div>
      </div>
      ...
      <div className="ahd-tabs">
        {TABS.map(t => (
          <button key={t.id}
            className={`ahd-tab ${activeTab === t.id ? "active" : ""}`}
              ...
              <div className="ahd-info-right-col">
                ...
                {hotel.description && (
                  <div className="ahd-card">
                    ...
                    <p className="ahd-desc" style={{ padding: "0 22px 18px" }}>
                      {hotel.description}
                    </p>
                  </div>
                )}
            ...
            {images.length > 0 && (
              <div className="ahd-card">
                ...
                <div className="ahd-photos-grid" style={{ padding: "0 16px 16px" }}>
                  {images.map((img, i) => (
                    <div key={img.id || i} className="ahd-photo"
                      ...
                      {img.type === "PRINCIPALE" && (
                        <span className="ahd-photo-badge">⭐ Principale</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="ahd-chambres-section">
            ...
            <div className="ahd-ch-filters">
              <div className="ahd-ch-search">
                ...
                <input
                  value={chambreSearch}
                  onChange={e => setChambreSearch(e.target.value)}
                ...
                {chambresFiltrees.map(ch => (
                  <div key={ch.id} className="ahd-chambre-card">
                    ...
                    <div className="ahd-chambre-body">
                      ...
                      {ch.description && (
                        <p className="ahd-chambre-desc">{ch.description}</p>
                      )}
                      {(ch.prix_min != null) && (
          ...
          <div className="ahd-dispo-section">
              ...
              <>
                ...
                <div className="ahd-dispo-grid">
                  {dispoResult.chambres?.map(ch => (
                    <div key={ch.chambre_id}
                      className={`ahd-dispo-card ${ch.disponible ? "dispo" : "occupe"}`}>
                      ...
                      <div className="ahd-dispo-card-body">
                        <div className="ahd-dispo-card-header">
                          <span className="ahd-chambre-type">
                            {ch.type_chambre?.nom || `Chambre #${ch.chambre_id}`}
                          </span>
            ...
            {!dispoResult && !dispoLoading && !dispoError && (
              <div className="ahd-empty">
                <span>📅</span>
                <p>Sélectionnez une période pour voir les disponibilités</p>
              </div>
            )}
          </div>
        )}
