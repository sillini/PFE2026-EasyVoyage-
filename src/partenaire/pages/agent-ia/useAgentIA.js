// ══════════════════════════════════════════════════════════
//  src/partenaire/pages/agent-ia/useAgentIA.js
//  Hook principal pour la gestion d'etat de l'Agent IA Partenaire
// ══════════════════════════════════════════════════════════
...
const WELCOME_MSG = {
  ...
  role: "assistant",
  contenu:
    "Bonjour 👋 Je suis votre Assistant IA EasyVoyage. Je peux vous aider à piloter votre activité : hôtels, chambres, réservations, finances, promotions… Comment puis-je vous aider aujourd'hui ?",
...
export function useAgentIA() {
  ...
  // ══════════════════════════════════════════════════════════
  //  ACTIONS — nouveau chat, selection, renommer, supprimer
  // ══════════════════════════════════════════════════════════
  ...
  const handleClearAll = useCallback(async () => {
    if (
      !window.confirm(
        "⚠️ Supprimer TOUTES vos conversations ? Cette action est irreversible."
      )
    )
      return;
    try {
  ...
  const handleSend = useCallback(
    async (text) => {
      ...
      // Optimistic UI : afficher le message user immediatement
      const tempUserMsg = {
      ...
      abortRef.current = await sendMessageStream(
        { message: msg, conversation_id: activeConvId },
        {
          ...
          onUserMsg: (userMsg) => {
            // Remplace le message temporaire par celui renvoye par le backend
            setMessages((prev) =>
          ...
          onDone: (data) => {
            ...
            setStreamingText("");
            // Rafraichir la sidebar pour voir le nouveau titre
            refreshSidebar();
          },
          onError: (err) => {
            setMessages((prev) => [
